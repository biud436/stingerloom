  ...
  public async create(
    ...
    metadata.routers.forEach(
      ({ method, path: routerPath, router, parameters }) => {
        const routeHandler: HttpHandler = async (context: HttpContext) => {
          const { request, response } = context;
          const req = request;
          const res = response;
          const bodyValidationActions: Promise<ValidationError[]>[] = [];
          const routerName = (router as (...args: unknown[]) => unknown).name;

          // Guard Consumer
          await this.guardConsumer.create(req, targetController, routerName);
          ...
          const result = (router as any).call(targetController, ...args);
        ...
        const registerPath = this.normalizePath(controllerPath, routerPath);
