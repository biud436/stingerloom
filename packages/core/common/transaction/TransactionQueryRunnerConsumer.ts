  ...
  public execute(
    ...
    const wrapper = async (...args: any[]) => {
      ...
      if (propagation === TransactionPropagation.REQUIRED) {
        ...
        throw new Exception("지원하지 않는 트랜잭션 전파 속성입니다", 500);
      }

      if (!queryRunner) {
        throw new Exception("트랜잭션 QueryRunner를 찾을 수 없습니다", 500);
      }
      ...
      try {
        ...
        if (propagation === TransactionPropagation.REQUIRED) {
           ...
           **/
          if (this.transactionScanner.isCommittedAllLogicalTransaction()) {
