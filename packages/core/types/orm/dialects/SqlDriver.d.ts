...
export interface ISqlDriver<T = any> {
     ...
     * @param columnType
     */
    addColumn(tableName: string, columnName: string, columnType: string): Promise<T>;
    /**
     * 테이블의 컬럼을 제거합니다.
     *
     * @param tableName
     ...
     */
    addForeignKey(tableName: string, columnName: string, foreignTableName: string, foreignColumnName: string): Promise<T>;
     ...
     * @param indexName
     */
    addIndex(tableName: string, columnName: string, indexName: string): Promise<T>;
     ...
     * @param columns
     */
    createTable(tableName: string, columns: Omit<ColumnMetadata, "target" | "type">[]): Promise<T>;
    /**
