...
import { RawTransactionScanner } from "@stingerloom/core/common/transaction/RawTransactionScanner";
...
export class DataSourceProxy {
      ...
      get: (target: DataSource, prop: string | symbol, receiver) => {
        ...
        if (
          ...
          return <Entity extends ObjectLiteral>(
            ...
            const repository = this.createRepositoryProxy(entity, target, prop);
  ...
  private createQueryRunner(dataSource: DataSource, target: DataSource) {
    ...
    if (!targetMethod) {
      throw new Error("cannot find createQueryRunner method in DataSource");
    }

    return (...args: unknown[]) => {
      const originalQueryRunner = targetMethod.apply(dataSource, args as []);
  ...
  private createEntityManagerProxy(manager: EntityManager) {
    return new Proxy(manager, {
      get: (target, prop, receiver) => {
        if (this.transactionScanner.isGlobalLock()) {
          const txEntityManager = this.transactionScanner.getTxEntityManager();

          if (!txEntityManager) {
            throw new Error("트랜잭션 EntityManager를 찾을 수 없습니다");
          }
